...
def player(prev_play, opponent_history=["R"]):
    ...
    encoder_input_data = np.array([int_history[-9:]]) 
    decoder_input_data = np.array([[move_to_int[prev_play]]]) 

    def my_model():
        
        model = keras.models.load_model('00Boilerplate-RPS/best_models/counter_abbey_6.keras') # Choose your model here
    ...
    decoder_output = loaded_model.predict([encoder_input_data, decoder_input_data]) #change the model between loaded_model or model based on who the player is going against
